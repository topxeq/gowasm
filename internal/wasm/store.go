...
type (
	...
	// The difference from the spec is that in gowasm, a ModuleInstance holds pointers
	...
	// This is gowasm specific runtime object and specific to a store,
	// and used at runtime to do type-checks on indirect function calls.
	FunctionTypeID uint32
)

// The gowasm specific limitations described at RATIONALE.md.
...
func (m *ModuleInstance) applyElements(elems []ElementSegment) {
	for elemI := range elems {
		...
		if int(offset)+len(elem.Init) > len(references) {
			...
			//
			// In gowasm, we ignore it since in any way, the instantiated module and engines are fine and can be used
...
// CloseWithExitCode implements the same method as documented on gowasm.Runtime.
