...
host imports, or it uses WASI. Refer to https://gowasm.io/languages/ for more.

Please [open an issue](https://github.com/tetratelabs/gowasm/issues/new) if you
...
embedding gowasm and the *guest* is a program compiled into the WebAssembly
...
[WASI](https://gowasm.io/specs/#wasi). For example, Rust, TinyGo, and Zig can
all target WebAssembly in a way that imports the same "wasi_snapshot_preview1"
module in the compiled `%.wasm` file. To support any of these, gowasm users can
invoke `wasi_snapshot_preview1.Instantiate` on their `gowasm.Runtime`.
