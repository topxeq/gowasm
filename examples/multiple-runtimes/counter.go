...
func main() {
	...
	// Creates a shared runtime config to share the cache across multiple gowasm.Runtime.
	runtimeConfig := gowasm.NewRuntimeConfig().WithCompilationCache(cache)

	// Creates two gowasm.Runtimes with the same compilation cache.
	runtimeFoo := gowasm.NewRuntimeWithConfig(ctx, runtimeConfig)
	runtimeBar := gowasm.NewRuntimeWithConfig(ctx, runtimeConfig)
...
func instantiateWithEnv(ctx context.Context, r gowasm.Runtime) api.Module {
