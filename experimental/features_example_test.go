...
func ExampleCoreFeaturesThreads() {
	...
	cfg := gowasm.NewRuntimeConfig().WithCoreFeatures(api.CoreFeaturesV2 | experimental.CoreFeaturesThreads)

	r := gowasm.NewRuntimeWithConfig(ctx, cfg)
	defer r.Close(ctx)
	...
	if _, err := r.InstantiateWithConfig(ctx, memoryWasm, gowasm.NewModuleConfig().WithName("env")); err != nil {
		log.Panicln(err)
	}

	mod, err := r.InstantiateModule(ctx, wasmCompiled, gowasm.NewModuleConfig().WithStartFunctions("_initialize"))
	if err != nil {
		log.Panicln(err)
	}
...
func createChildModule(rt gowasm.Runtime, root api.Module, wasmCompiled gowasm.CompiledModule) *childModule {
	childModuleMu.Lock()
	...
	child, err := rt.InstantiateModule(ctx, wasmCompiled, gowasm.NewModuleConfig().
...
func ExampleCoreFeaturesTailCall() {
	...
	cfg := gowasm.NewRuntimeConfig().WithCoreFeatures(api.CoreFeaturesV2 | experimental.CoreFeaturesTailCall)
